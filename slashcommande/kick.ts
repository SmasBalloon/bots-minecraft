...
export const killServer = new SlashCommandBuilder()
  .setName("kick")
  .setDescription("kick un joueur du serveur Minecraft");
killServer.addStringOption(option =>
  option
    .setName("pseudo")
    .setDescription("Pseudo du joueur (in game) à kicker")
    .setRequired(true)
);
...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    try {
      ...
      // Envoyer la commande kick
      const response = await rcon.send(`kick ${pseudo}`);

      await interaction.editReply({
        content: `✅ Le joueur **${pseudo}** a été kické sur le serveur !`
      });
